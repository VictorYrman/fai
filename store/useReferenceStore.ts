...
export const useReferenceStore = create<ReferenceState>((set, get) => ({
  ...
  getExerciseCategories: async () => {
    ...
    if (currentExerciseCategories && currentExerciseCategories.length > 0)
      return;
    ...
    try {
      ...
      const exercises = rawExercises?.map((exercise) => {
        return {
          ...
          exerciseCategory: get().getExerciseCategory(
            exercise.exerciseCategoryId,
          ),
          muscleCategories: exercise.muscleCategories.map(
            (muscleCategory: any) => ({
              isPrimary: muscleCategory.isPrimary,
              muscleCategory: get().getMuscleCategory(
                muscleCategory.muscleCategoryId,
              ),
            }),
          ),
          difficultyLevel: get().getDifficultyLevel(exercise.difficultyLevelId),
          goals: exercise.goalIds.map((goalId: string) =>
            get().getGoal(goalId),
          ),
          impactPoints: exercise.impactPointIds.map((impactPointId: string) =>
            get().getImpactPoint(impactPointId),
          ),
  ...
  getExerciseCategory: (id: string) => {
    return get().exerciseCategories.find(
      (exerciseCategory) => exerciseCategory.id === id,
    );
  },
  getMuscleCategory: (id: string) => {
    return get().muscleCategories.find(
      (muscleCategory) => muscleCategory.id === id,
    );
  },
  getDifficultyLevel: (id: string) => {
    return get().difficultyLevels.find(
      (difficultyLevel) => difficultyLevel.id === id,
    );
  },
  getGoal: (id: string) => {
